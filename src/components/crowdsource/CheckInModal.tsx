...
export const CheckInModal: React.FC<CheckInModalProps> = ({
  ...
  // Minimal focus trap: wrap Tab cycling inside the modal
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setNote("");
      // Prefill density/AC from the target vehicle's current live values
      const target = simulatedVehicles.find((v) => v.id === activeId);
      if (target) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      prevVehicleRef.current = currentVehicle ?? null;
      ...
      if (prevVehicleRef.current) {
        updateSingleVehicle(prevVehicleRef.current);
      }
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          <button
            onClick={onClose}
                         className={`touch-target focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/70 p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-white/10 btn-tactile transition-colors`}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5 no-scrollbar">
          {/* Success Banner */}
          {isSubmitted ? (
                        <div role="status" className="p-5 rounded-xl bg-emerald-950/60 border border-emerald-500/40 text-center space-y-3">
            ...
            <>
              ...
              <div className="space-y-1.5">
                                <label htmlFor="checkin-vehicle" className="text-xs font-semibold text-slate-300 flex items-center justify-between">
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-300 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-400">{t.crowdsource.tapToSelect}</span>
                                ...
                                <div
                  role="radiogroup"
                  aria-label={t.crowdsource.densityLevelLabel}
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-300 flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-slate-400">{t.crowdsource.cabinHint}</span>
                                ...
                                <div
                  role="radiogroup"
                  aria-label={t.vehicleInspector.acComfort}
              ...
              <div className="space-y-1.5">
                                 <label htmlFor="checkin-note" className="text-xs font-semibold text-slate-300 flex items-center justify-between">
                   <span className="flex items-center gap-1.5">
                  ...
                  placeholder={t.crowdsource.notePlaceholder}
                                     className={`touch-target w-full px-3 py-2 rounded-xl bg-slate-900/90 border border-slate-700/80 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-cyan-500 transition`}
                />
              </div>
          ...
          <div className="px-5 py-3.5 border-t border-white/10 bg-slate-900/80 flex items-center justify-between shrink-0">
            <button
              ...
              disabled={isSubmitting}
                             className={`touch-target focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/70 px-4 py-2 rounded-xl border border-slate-700 text-xs font-medium text-slate-300 hover:bg-slate-800 btn-tactile transition`}
            >
              {t.common.cancel}
            </button>
